    ...
    while True:
        ...
        if 1:
          ...
          if 1:
              ...
              wordact, _ = lang_model(labels.view(batchsize, -1))
              wordact_t = wordact[:, :-1, :]
              wordact_t = wordact_t.contiguous().view(wordact_t.size(0) * wordact_t.size(1), -1)
              labels_flat = labels.view(batchsize, -1)
              wordclass_v = labels_flat[:, :]
              wordclass_t = wordclass_v.contiguous().view( \
                  wordclass_v.size(0) * wordclass_v.size(1), -1)
              loss_xe_lang = F.cross_entropy(wordact_t[...], wordclass_t.view(-1))

              outcap, sample_logprobs, x_all_langauge, sampled_ids = lang_model.sample(labels.view(batchsize, -1), loader.get_vocab())
              with torch.no_grad():
                  reward, cider_sample, cider_greedy = get_self_critical_reward(batchsize, lang_model, labels.view(batchsize, -1), outcap,
                                                                                captions_all, loader,
                                                                                180)
              ...
              train_loss = loss_xe + loss_rl + l2_loss + loss_xe_lang
              train_loss.backward()
              optimizer.step()
