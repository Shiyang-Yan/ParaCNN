...
def train(opt):
    ...
    if os.path.isfile(os.path.join('log_cvpr/',"optimizer.pth")):
          optimizer.load_state_dict(torch.load(os.path.join('log_cvpr/', 'optimizer.pth')))
    ...
    while True:
        ...
        if (iteration % opt.save_checkpoint_every == 0):
            checkpoint_path = os.path.join(opt.checkpoint_path, str(iteration)+'model2.pth')
...
opt.id = 'xe'
opt.print_freq = 100
opt.model = 'log_cvpr/18000model.pth'
