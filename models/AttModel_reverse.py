...
class convcap(nn.Module):
  ...
  def __init__(self, opt, reverse = False):
    ...
    if not self.reverse:
        self.fc_forward = Linear(512, 512)
  ...
  def forward(self, non_used, imgsfeats, wordclass):
    if 1:
      ...
      if len(output.size())>2:
            ...
            x_all_output = x_all_output.transpose(2,1).contiguous()
      else:
            output = output.transpose(1,0).unsqueeze(0)
            x_all_output = x_all_output.transpose(1,0).unsequeeze(0)
      output = output.view((batchsize, 8668, -1))
      x_all_output = x_all_output.view((batchsize, 512, -1)).contiguous()

      if not self.reverse:
          x_all_output = x_all_output.transpose(2,1).contiguous().view((-1, 512))
          x_all_output = self.fc_forward(x_all_output)
          x_all_output = x_all_output.view((batchsize, -1, 512))
          x_all_output = x_all_output.transpose(2,1).contiguous()
