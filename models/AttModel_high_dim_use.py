...
class convcap(nn.Module):
  ...
  def __init__(self, opt):
    ...
    nfeats = 1024
    dropout = .1
    self.fusion = Linear(1024, 512)
    self.fc_imgfeats = Linear(2048, nfeats)
    self.fc_imgfeats2 = Linear(1024, 512)
    self.nimgfeats = 2048
    ...
    self.topic_proj = Linear(self.nfeats*2, self.nfeats)
    self.imgproj = Linear(self.nfeats//2, self.nfeats//2, dropout=dropout)
    self.resproj = Linear(nfeats//2, self.nfeats//2, dropout=dropout)
    self.resproj_para = Linear(nfeats, self.nfeats//2 ,dropout=dropout)
    n_in = self.nfeats
    n_out = self.nfeats
    ...
    for i in range(4):
      self.convs_topic.append(Conv1d(n_in//2, n_out, self.kernel_topic, self.pad_topic))
      n_in = n_out

    self.classifier_0 = Linear(self.nfeats//2, (nfeats))
    #self.bn1 = nn.BatchNorm1d(num_features = (nfeats // 2))
    self.classifier_1 = Linear((nfeats), num_wordclass, dropout=dropout)
  ...
  def forward(self, non_used, imgsfeats, wordclass, length, num):
    ...
    para_level_visual = imgsfc7.unsqueeze(2).expand(imgsfc7.size(0), self.nfeats, num)
    x_all = torch.zeros((batchsize, length, num , 2048//2))
    x_all_out = torch.zeros((batchsize, length, num, 2048//2))
    for index in range(num):
      ...
      x = self.conv_cap(x, wordemb, F.relu(self.fc_imgfeats2(imgsfeats_used)))
    ...
    x_all_out = x_all_out.view((batchsize, 2048, -1)).contiguous()
