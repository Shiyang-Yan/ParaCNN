  ...
  def forward(self, non_used, imgsfeats, wordclass):
    ...
    for index in range(6):
      ...
      if len(wordemb.size())>2:
        x = wordemb.transpose(2, 1).cuda()  
        batchsize, wordembdim, maxtokens = x.size()
      else:
        x = wordemb.transpose(1, 0).cuda().unsqueeze(0)
        batchsize, wordembdim, maxtokens = x.size()
