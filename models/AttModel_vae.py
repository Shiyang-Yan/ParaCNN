...
class convcap(nn.Module):
  ...
  def __init__(self, opt):
    ...
    nfeats = 1024
    dropout = .1
    self.fc_imgfeats = Linear(2048, 256)
    self.nimgfeats = 2048
    ...
    self.fc1 = Linear(256, 20)
    self.fc2 = Linear(256, 20)
    self.fc3 = Linear(20, 256)
  ...
  def forward(self, non_used, imgsfeats, wordclass):
    ...
    mu = self.fc1(imgsfc7)
    logvar = self.fc2(imgsfc7)
    z = F.relu(self.fc3(self.reparameterize(mu,logvar)))
    para_level_visual = torch.cat([imgsfc7, z], 1)
    para_level_visual = para_level_visual.unsqueeze(2).expand(z.size(0), self.nfeats//2, 6)
