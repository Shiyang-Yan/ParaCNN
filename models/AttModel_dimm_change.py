...
class convcap(nn.Module):
  ...
  def __init__(self, opt):
    ...
    self.emb_0 = Embedding(num_wordclass, nfeats//4, padding_idx=0)
    self.emb_1 = Linear(nfeats//4, nfeats//4, dropout=dropout)
    self.topic_proj = Linear(self.nfeats//4*3, self.nfeats//4)
    self.imgproj = Linear(self.nfeats//4, self.nfeats//4, dropout=dropout)
    self.resproj = Linear(nfeats//2, self.nfeats//4, dropout=dropout)
    self.resproj_para = Linear(nfeats//4, self.nfeats//4 ,dropout=dropout)
    n_in = self.nfeats
    n_out = self.nfeats//2
    ...
    self.classifier_0 = Linear(self.nfeats//4, (nfeats // 4))
    #self.bn1 = nn.BatchNorm1d(num_features = (nfeats // 2))
    self.classifier_1 = Linear((nfeats // 4), num_wordclass, dropout=dropout)
  ...
  def forward(self, non_used, imgsfeats, wordclass):
    ...
    para_level_visual = imgsfc7.unsqueeze(2).expand(imgsfc7.size(0), self.nfeats//2, 6)
    x_all = torch.zeros((batchsize, 30, 6, 512))
    x_all_out = torch.zeros((batchsize, 30, 6, 512))
    ...
    #for index in range(6):
      ...
      #dis = F.relu(self.fc1(topic))
      #dis_all[:, index] = F.sigmoid(self.fc2(dis)).squeeze().cpu()


      y = F.relu(self.imgproj(topic))
      
      y = y.unsqueeze(2).expand(batchsize, self.nfeats//4, maxtokens)
      x = torch.cat([x, y], 1)
    ...
    x_all_out = x_all_out.view((batchsize, 1024, -1)).contiguous()
