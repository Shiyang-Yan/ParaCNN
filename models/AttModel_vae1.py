...
class convcap(nn.Module):
  ...
  def __init__(self, opt):
    ...
    self.fc2 = Linear(512, 32)
    self.fc3 = Linear(32, 512)
  ...
  def forward(self, non_used, imgsfeats, wordclass):
    ...
    mu_all = torch.zeros((batchsize, 6, 32))
    logvar_all = torch.zeros(batchsize, 6, 32)
    ...
    for index in range(6):
      ...
      logvar =self.fc2(y)
      mu_all[:,index, :] = mu
      logvar_all[:, index, :] = logvar
      z = F.relu(self.fc3(self.reparameterize(mu,logvar)))
     
      y = z.unsqueeze(2).expand(batchsize, self.nfeats//2, maxtokens)
      x = torch.cat([x, y], 1)
    ...
    return output, mu_all.mean(1), logvar_all.mean(1)
