...
class convcap(nn.Module):
  def __init__(self, opt):
    ...
    self.encoder = MemoryAugmentedEncoder(3, 1, attention_module=ScaledDotProductAttentionMemory,
                                     attention_module_kwargs={'m': 40})
    self.decoder = MeshedDecoder(8668, 30, 3, 1, self.emb_0)
    self.meshed_model = Transformer(8667, self.encoder, self.decoder)
  ...
  def forward(self, non_used, imgsfeats, wordclass, length, num):
    ...
    for index in range(num):
      ...
      x, x2 = self.meshed_model(F.relu(imgsfeats_used + y.mean(2).unsqueeze(1)), wordclass[:,index,:])
      x_all_out[:,:,index,:] = x2

      output[:, index,:] = x.transpose(-2,-1)
