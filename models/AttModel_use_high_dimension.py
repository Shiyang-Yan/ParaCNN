...
class convcap(nn.Module):
  ...
  def __init__(self, opt):
    ...
    self.resproj_para = Linear(nfeats, self.nfeats ,dropout=dropout)
    n_in = 2*self.nfeats
    n_out = self.nfeats
    ...
    for i in range(self.n_layers):
      self.convs.append(Conv1d(n_in, n_out*2, self.kernel_size, self.pad, dropout))
    ...
    for i in range(4):
      self.convs_topic.append(Conv1d(n_in, n_out*2, self.kernel_topic, self.pad_topic))
      n_in = n_out

    self.classifier_0 = Linear(self.nfeats, (nfeats))
  ...
  def forward(self, non_used, imgsfeats, wordclass, length, num):
    ...
    context = torch.zeros((batchsize, 2048//2))
