...
class convcap(nn.Module):
  ...
  def __init__(self, opt):
    ...
    self.maxes = MaxPool1d()
    n_in = 2*self.nfeats
    n_out = self.nfeats
    self.n_layers = num_layers
    self.convs = nn.ModuleList()
    self.conv_recs = nn.ModuleList()
    ...
    for i in range(4):
      if i == 3:
        n_out = 4096
      self.conv_recs.append(Conv1d(n_in//2, n_out, self.kernel_recs, 0))
      n_in = n_out
  ...
  def conv_reconstructs(self, x):
    for i, conv in enumerate(self.conv_recs):
        x = conv(x)
        if i % 2 == 0 and i<2:
          x = self.maxes(x)
        x = F.glu(x, dim=1)
  ...
  def forward(self, non_used, imgsfeats, wordclass):
    ...
    #dis = F.relu(self.fc1(topic_conv.transpose(2,1)))
    #dis_all = F.sigmoid(self.fc2(dis)).cpu()
    ...
    #for index in range(6):
      ...
      reconstruct = self.conv_reconstructs(x2.transpose(2,1))
     
      reconstruct, _ = reconstruct.max(2)
