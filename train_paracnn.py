...
#writer = SummaryWriter("my_experiments")
...
def train(opt):
    ...
    # Load the optimizer
    if  os.path.isfile(os.path.join('log_xe/',"optimizer.pth")):
        optimizer.load_state_dict(torch.load(os.path.join('log_xe/', 'optimizer.pth')))
    ...
    while True:
        ...
        optimizer.zero_grad()
        if not sc_flag:
            wordact, _ = dp_model(fc_feats, att_feats, labels, 30, 6, att_masks)
            p = F.softmax(wordact,1)
            logp = F.log_softmax(wordact,1)
            entropy = -(p*logp).mean(2).sum(1).mean(0)
...
#        utils.clip_gradient(optimizer, opt.grad_clip)
        ...
        if iteration % opt.print_freq == 1:
            ...
            if not sc_flag:
                print("iter {} (epoch {}), train_loss = {:.3f}, entropy = {:.3f}, data_time = {:.3f}, time/batch = {:.3f}" \
