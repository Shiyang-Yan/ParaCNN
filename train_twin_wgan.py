...
import opts
from models.transformer_revise.transformer import *
        ...
        if 1:
          if iteration>=0:
            ...
            if 1:
              ...
              back_model.eval()
              with torch.no_grad():
                _, x_all_d = dp_model(fc_feats, att_feats, nd_labels.long(), 30, 6)
            

                labels_nd = nd_labels.view(batchsize,-1)
                idx = [i for i in range(labels_nd.size()[1] - 1, -1, -1)]
                labels_flip_nd = labels_nd[:, idx]
                labels_flip_nd = labels_flip_nd.view(batchsize, 6, 30)
                labels_flip_nd[:,:,0] = 8667
                _, x_all_flip_d = back_model(fc_feats, att_feats, labels_flip_nd, 30, 6)

                x_all_d = x_all_d[:,:,:-1]
                x_all_flip_d = x_all_flip_d[:,:,:-1]


                idx = [i for i in range(x_all_flip_d.size()[2] - 1, -1, -1)]
              ...
              autoregressive_scores = dis_model(x_all_d.transpose(2, 1).cuda())
              teacher_forcing_scores = dis_model(invert_backstates.transpose(2, 1).cuda())

              tf_loss, ar_loss = _calcualte_discriminator_loss(teacher_forcing_scores, autoregressive_scores)
            ...
            if 1:
              ...
              labels_flip[:,:,0] = 8667
              wordact, x_all_flip = back_model(fc_feats, att_feats, labels_flip, 30, 6)
              ...
              autoregressive_scores = dis_model(x_all.transpose(2,1).cuda())

              ad_loss = _calculate_generator_loss(autoregressive_scores).sum()
...
#            utils.clip_gradient(optimizer, opt.grad_clip)
        ...
        # Validate and save model 
          if (iteration % opt.save_checkpoint_every == 0):
            ...
            checkpoint_path = os.path.join(opt.checkpoint_path, 'all2d_model{:05d}.pth'.format(iteration))
            torch.save(back_model.state_dict(), checkpoint_path)
            print("model saved to {}".format(checkpoint_path))
            checkpoint_path = os.path.join(opt.checkpoint_path, 'all2dis_model{:05d}.pth'.format(iteration))
