...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    # Make sure in the evaluation mod
    ...
    while True:
        ...
        with torch.no_grad():
              seq_logprobs = model(fc_feats, att_feats, length, num, batchsize, None)
              #
        ...
        for k, sent in enumerate(sents):
            sent = sent.replace('<start>', '').replace(' .', '.').replace('UNK', '').replace('<pause>', '').replace('  ',' ')
